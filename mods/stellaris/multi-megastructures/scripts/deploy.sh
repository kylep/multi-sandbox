#!/usr/bin/env bash
# Deploy the Multi-Megastructures mod into the local Stellaris user dir.
#
# Creates two artifacts in `~/Documents/Paradox Interactive/Stellaris/mod/`:
#   1. A symlink from `multi-megastructures/` to this repo's `mod/` folder, so
#      `scripts/build.py` re-runs are picked up by the launcher without re-deploy.
#   2. A `multi-megastructures.mod` launcher descriptor with an absolute `path=`
...
echo
echo "Next: launch Stellaris -> Paradox Launcher -> Mods -> enable 'Multi-Megastructures + Free Tech'"
