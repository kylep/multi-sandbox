# multi-sandbox

Experiments, half-baked ideas, and archived projects. **Nothing here is
maintained.** No quality bar, no branch protection, no dependency
hygiene, no promises — commit straight to `main`.

Actively maintained work lives in [kylep/multi](https://github.com/kylep/multi),
which is treated as production: PRs to main, accurate docs, zero-vuln
policy. When a project there stops being maintained it migrates here;
if something here graduates, it moves to `multi` and gets brought up to
that standard.

Most projects arrived in the 2026-07-04 demotion from `multi`
(kylep/multi PR: "demote unmaintained projects to multi-sandbox").
Their full git history remains in `multi`'s history.

## Contents

| Dir | What it is |
|-----|------------|
| `agent-loops/` | macbook-security-loop experiment |
| `branch-header-extension/` | Browser extension experiment |
| `games/` | kid-bot-battle-sim, robotext-battle, sillyapp, snake (robot-battle, the maintained web game, stayed in multi) |
| `macos-utils/` | macOS utility scripts |
| `mods/` | Old Minecraft mod (the maintained Stellaris/HOI4 mods stayed in multi) |
| `openclaw-skills/` | OpenClaw skills experiment |
| `infra/aws/` | Old AWS infra |
| `infra/mac-setup/` | Mac setup scripts |
| `infra/openclaw-k8s/` | OpenClaw k8s manifests (never deployed to the current cluster) |
| `infra/ai-lint-toolkit/` | Lint toolkit container experiment |
| `attic/` | Loose artifacts from multi's repo root: old plans (`docs/`), TODO.md, prompt-readme.md, stale gitlab-ci.yml |
